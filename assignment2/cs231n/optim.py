...
def sgd(w, dw, config=None):
    ...
    # SGD is based on regular gradient descent with 
    # batch_size equals 1.
    # so its formula is
...
def adam(w, dw, config=None):
    ...
    eps = config["epsilon"]
    first_moment = b1 * first_moment + (1-b1)*dw
    second_moment = b2 * second_moment + (1-b2)*(dw**2)

    first_ub = first_moment / (1-b1**t+eps)
    second_ub = second_moment / (1-b2**t+eps)

    w -=lr*first_ub / (np.sqrt(second_ub)+eps)
